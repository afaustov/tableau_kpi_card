...
async function refreshKPIs(worksheet) {
  ...
  try {
    ...
    showDebug('📊 Starting bar chart fetch...');
...
// OPTIMIZED Bar Chart Data Fetching with Visual Debug Logs
async function fetchBarChartData(worksheet, dateFieldName, metricField, range) {
  try {
    ...
    console.log(`📊 [${metricField}] Fetching ${daysDiff} days: ${startDate.toISOString().split('T')[0]} to ${endDate.toISOString().split('T')[0]}`);
    showDebug(`🔄 [${metricField}] Fetching ${daysDiff} days...`);
    ...
    console.log(`📊 [${metricField}] Summary rows: ${summary.data.length}, dateCol: ${dateIndex}, metricCol: ${metricIndex}`);
    showDebug(`📊 [${metricField}] ${summary.data.length} rows, dateIdx=${dateIndex}, metricIdx=${metricIndex}`);

    if (dateIndex === -1 || metricIndex === -1) {
      console.warn(`⚠️ [${metricField}] Column not found`);
      showDebug(`❌ [${metricField}] Column not found!`);
      await worksheet.clearFilterAsync(dateFieldName);
      return [];
    }

    // Group data by date on client-side using Map
    ...
    console.log(`📊 [${metricField}] Grouped ${dailyDataMap.size} dates`);
    showDebug(`✅ [${metricField}] Grouped ${dailyDataMap.size} dates`);

    if (dailyDataMap.size > 0) {
      const sampleKeys = Array.from(dailyDataMap.entries()).slice(0, 3);
      console.log(`📊 [${metricField}] Sample:`, sampleKeys.map(([k, v]) => `${k}: ${v.toFixed(1)}`).join(', '));
      showDebug(`📊 Sample: ${sampleKeys.map(([k, v]) => `${k.slice(5)}: ${v.toFixed(0)}`).join(', ')}`);
    }
    ...
    console.log(`📊 [${metricField}] ${dataPoints.length} points (${nonZeroCount} non-zero)`);
    showDebug(`✅ [${metricField}] ${dataPoints.length} points (${nonZeroCount} with data)`);
    ...
    showDebug(`❌ [${metricField}] ERROR: ${e.message}`);
    return [];
  }
}
